...
def convert(in_path: Path, out_path: Path,
            min_ply: int, max_ply: int, max_score: int) -> None:
    games = 0
    positions = 0

    with open(in_path) as fin, open(out_path, 'w') as fout:
        fout.write('fen\tscore_cp\twdl\n')
        ...
        for raw in fin:
            ...
            wdl    = _result_wdl(rec.get('result', '*'))
            ...
            for ply, move in enumerate(moves):
                ...
                if ply >= min_ply and score is not None and abs(score) <= max_score:
                    ...
                    fout.write(f'{board.to_fen()}\t{score_white}\t{wdl}\n')
                    positions += 1
    ...
    if games:
        print(f'Per game  : {positions / games:.1f}')
...
def main() -> None:
    ...
    convert(in_path, Path(args.output), args.min_ply, args.max_ply, args.max_score)
